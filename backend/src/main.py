...
async def websocket_endpoint(websocket: WebSocket, usuario: str):
    ...
    try:
        while True:
            # Mantenemos la conexión abierta escuchando mensajes (si llegan)
            data = await websocket.receive_text()
    except WebSocketDisconnect:
        manager.disconnect(usuario)

# ==========================================
# ENDPOINTS: TAREAS COMPARTIDAS
# ==========================================

@app.get("/api/tareas", response_model=List[schemas.TareaResponse], tags=["Tareas Compartidas"])
def obtener_tareas(db: Session = Depends(get_db)):
    return db.query(models.Tarea).order_by(models.Tarea.id.asc()).all()

@app.post("/api/tareas", response_model=schemas.TareaResponse, tags=["Tareas Compartidas"])
...
@app.put("/api/tareas/{tarea_id}", response_model=schemas.TareaResponse, tags=["Tareas Compartidas"])
...
@app.delete("/api/tareas/{tarea_id}", tags=["Tareas Compartidas"])
